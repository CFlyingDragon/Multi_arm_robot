...
from plot_armctr_form import Ui_PlotArmctrForm
...
#***************************armctr多臂绘图窗口********************************#
class PlotArmctrWindow(QMainWindow, Ui_PlotArmctrForm):
    #建立全局变量
    num = 500
    state_qq_list1 = list(np.zeros([num, 6]))
    state_qq_list2 = list(np.zeros([num, 7]))
    state_qq_list3 = list(np.zeros([num, 7]))
    state_f_list1 = list(np.zeros([num, 6]))
    state_f_list2 = list(np.zeros([num, 6]))
    state_f_list3 = list(np.zeros([num, 6]))
    state_t_list = list(np.zeros(num))
    state_t = 0

    #构造函数
    def __init__(self, parent=None):
        super(PlotArmctrWindow, self).__init__(parent)
        self.T = 0.01
    ...
    def initUI(self):
        ...
        #创建菜单
        menubar = self.menuBar()
        mainMenu = menubar.addMenu('&主界面')

        #-------------------armc菜单-------------------#
        #armc1机械臂
        mainForm = QAction(QIcon('exit.png'), '返回主界面', self)
        mainForm.setShortcut('Ctrl+o')
        mainForm.setStatusTip('返回主界面')
        mainForm.triggered.connect(self.gotoMain)
        mainMenu.addAction(mainForm)
    ...
    def storage_begin(self):
        #获取写入地址
        self.storage_qq1_path = str(self.lineEdit_pos_armr.text())
        self.storage_f1_path = str(self.lineEdit_force_armr.text())
        self.storage_qq2_path = str(self.lineEdit_pos_armt.text())
        self.storage_f2_path = str(self.lineEdit_force_armt.text())
        self.storage_qq3_path = str(self.lineEdit_pos_armc.text())
        self.storage_f3_path = str(self.lineEdit_force_armrc.text())
    ...
    def storage_end(self):
        ...
        qq_data2 = np.array(self.storage_qq2)
        force_data2 = np.array(self.storage_f2)
        qq_data3 = np.array(self.storage_qq3)
        force_data3 = np.array(self.storage_f3)
        ...
        fo.write(qq_data2, self.storage_qq2_path)
        fo.write(force_data2, self.storage_f2_path)
        fo.write(qq_data3, self.storage_qq3_path)
        fo.write(force_data3, self.storage_f3_path)

        msg = "数据已写入：/home/d/catkin_ws/src/robot_bag/armctr\n"
        self.textEdit.setText(msg)
    ...
    def realtime_plot(self):
        ...
        if (self.force_flag):
            ...
            plot_f1 = np.array(self.state_f_list1)
            plot_f2 = np.array(self.state_f_list2)
            plot_f3 = np.array(self.state_f_list3)
            self.plot_force1(plot_t, plot_f1)
            self.plot_force2(plot_t, plot_f2)
            self.plot_force3(plot_t, plot_f3)

        else:
            plot_t = np.array(self.state_t_list)
            plot_qq1 = np.array(self.state_qq_list1)
            plot_qq2 = np.array(self.state_qq_list2)
            plot_qq3 = np.array(self.state_qq_list3)
            self.plot_joint1(plot_t, plot_qq1)
            self.plot_joint2(plot_t, plot_qq2)
            self.plot_joint3(plot_t, plot_qq3)

    def run_topic(self):
        # 运行话题
        rospy.init_node('upper_plot_node')
