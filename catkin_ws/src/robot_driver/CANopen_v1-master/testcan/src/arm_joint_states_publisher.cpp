/*arm_joint_states_publisher.cpp 接收机械臂的can帧，转发为JointState*/
...
void JointState::PosFeedCallback(const testcan::Frame::ConstPtr &can_msg){                                 //接受到的数据帧
    ...
    if(!flag[pos_dev_id-1])                                                                                //是否有初始位置
    {
        ...
        if(abs(angleToZeroPosition[0])>10) flag[0]=0;        //新增，出现不可能的值，抛弃，解决1关节初始值不稳定
