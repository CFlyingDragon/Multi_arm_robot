...
#include <armc_controller/armc_effort_position_controller.h>
 ...
 */
 //只要填充init、stop、updata、begin
  ArmcEffortPositionController::ArmcEffortPositionController() {}
  ArmcEffortPositionController::~ArmcEffortPositionController() {sub_command_.shutdown();}

  bool ArmcEffortPositionController::init(hardware_interface::EffortJointInterface* hw, ros::NodeHandle &n)
  {
    ...
    sub_command_ = n.subscribe<std_msgs::Float64MultiArray>("command", 1, &ArmcEffortPositionController::commandCB, this);
    return true;
  }

  //在update中写如控制指令，
  void ArmcEffortPositionController::update(const ros::Time& time, const ros::Duration& period)
  {
  ...
  void ArmcEffortPositionController::commandCB(const std_msgs::Float64MultiArrayConstPtr& msg)      //回调函数
  {
  ...
  void ArmcEffortPositionController::enforceJointLimits(double &command, unsigned int index)        //关节限制
  {
...
PLUGINLIB_EXPORT_CLASS(armc_controller::ArmcEffortPositionController, controller_interface::ControllerBase)
